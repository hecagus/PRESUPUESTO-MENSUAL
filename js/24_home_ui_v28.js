/* v2.8.0 - UI de Hogar basada en significado financiero, no sólo frecuencia. */
...
export function renderHome(){
  ...
  zone.innerHTML=section('🔴 Obligaciones',items.filter(x=>x.kind==='obligation'))+section('🟡 Presupuestos necesarios',items.filter(x=>x.kind==='budget'))+section('🟠 Reservas / necesidades próximas',items.filter(x=>x.kind==='reserve'))+section('🔵 Opcionales',items.filter(x=>x.kind==='optional'));
