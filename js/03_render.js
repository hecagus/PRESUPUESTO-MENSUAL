...
const setupMobileMenu = () => {
    ...
    if (btn && nav) {
        ...
        newBtn.onclick = (e) => { e.stopPropagation(); nav.classList.toggle('active'); };
...
export const renderWalletUI = () => {
    ...
    if (container) {
        ...
        data.sobres.forEach(s => {
            ...
            div.innerHTML = `
                ...
                <div style="font-size:0.9rem; color:#64748b; margin-bottom:8px;">Guardar diario: <b>$${fmtMoney(s.diario)}</b></div>
        ...
        if (data.sobres.length === 0) container.innerHTML = "<p class='nota' style='text-align:center;'>No hay sobres activos.</p>";
    }
    ...
    if (elHealth) {
        ...
        elHealth.innerText = diff >= 0 ? `👍 Superávit: +$${fmtMoney(diff)}` : `⚠️ Déficit: -$${fmtMoney(Math.abs(diff))}`;
        elHealth.style.color = diff >= 0 ? "#86efac" : "#fca5a5"; 
    }
};
...
export const renderMantenimientoUI = () => {
    ...
    if (div) {  
        ...
        if (!r.alertaActiva) { div.style.cssText = "background: #ecfdf5; border: 1px solid #34d399;"; div.innerHTML = "Estado: 🟢 OK. ¡Servicios al día!"; } 
            ...
            let p = []; for (const i in r.alerta) { if (r.alerta[i]) { const k = safeNumber(r.kmRestantes[i]); const sim = k <= 0 ? '🔴' : '🟠'; p.push(`${sim} ${i} (${k <= 0 ? 'Excedido' : k + ' KM restantes'})`); } } 
            div.style.cssText = "background: #fee2e2; border: 1px solid #f87171;"; div.innerHTML = "⚠️ SERVICIO PENDIENTE: " + p.join('; '); 
        }  
    }
};
...
export const renderListasAdmin = () => {
    ...
    if (ul) { 
        ...
        const t = $("totalFijoMensualDisplay"); if(t) t.innerText = `$${fmtMoney(Data.getState().parametros.gastoFijo * 30)}`; 
    }
...
window.eliminarFijo = (index) => {
    if(confirm("¿Eliminar este gasto fijo permanentemente?")) { Data.eliminarGastoFijo(index); renderListasAdmin(); renderMetaDiaria(); }
};
...
export const renderDashboard = () => {
    ...
    const neta = bruta - gastos; const gph = (horas > 0) ? (bruta / horas) : 0;  
...
export const renderHistorial = () => {
    ...
    movs.forEach(m => { const isI = m.tipo === 'ingreso'; const mo = safeNumber(m.monto); if (isI) i += mo; else g += mo; tb.innerHTML += `<tr><td>${isI?'➕':'➖'}</td><td>${formatearFecha(m.fecha)}</td><td>${m.desc}</td><td style="color:${isI?'#16a34a':'#dc2626'}">$${fmtMoney(mo)}</td></tr>`; });  
...
export const setupAdminListeners = () => {
    ...
    safeClick("btnConfirmarFinalizar", () => { const m = $("gananciaBruta").value; const km = $("kmFinalTurno").value; if(!m) return alert("Ingresa la ganancia"); Data.finalizarTurnoLogic(m, km); renderTurnoUI(); renderOdometroUI(); $("gananciaBruta").value=""; $("kmFinalTurno").value=""; alert("Turno Finalizado"); });  
    ...
    const fill = (t) => { const s = $("gastoCategoriaSelect"); const man = $("gastoCategoriaManual"); if(!s) return; s.innerHTML=""; (CATEGORIAS_GASTOS[t]||[]).forEach(c => { const o = document.createElement("option"); o.value=c; o.text=c; s.add(o); }); s.onchange = () => { if (man) man.style.display = s.value.includes("➕") ? "block" : "none"; }; };  
    ...
    safeClick("btnRegistrarGasto", () => {
        const s=$("gastoCategoriaSelect"), man=$("gastoCategoriaManual"), m=$("gastoCantidad").value;
        if(!m) return alert("Falta monto");
        let c = (s.value.includes("➕") && man && man.value.trim()) ? man.value.trim() : s.value;
        const fix = $("checkEsRecurrente")?.checked || false;
        let t = "moto"; const rad = document.getElementsByName("gastoTipoRadio"); for(let r of rad) if(r.checked) t = r.value;
        const g = { id:Date.now(), fecha:new Date().toISOString(), categoria:c, monto:Number(m), desc:$("gastoDescripcion").value, tipo:t };
        if(fix) { g.frecuencia = $("gastoFrecuenciaSelect").value; Data.agregarGastoFijo(g); } 
        else { g.frecuencia = "No Recurrente"; Data.agregarGasto(g); }
        alert("Guardado"); $("gastoCantidad").value=""; $("gastoDescripcion").value=""; renderListasAdmin(); renderMetaDiaria();
    });
    safeClick("btnRegistrarIngreso", () => { const d = $("ingresoDescripcion").value, m=$("ingresoCantidad").value; if(m) { Data.getState().movimientos.push({tipo:'ingreso', fecha:new Date().toISOString(), desc:d||"Extra", monto:safeNumber(m)}); Data.saveData(); alert("Registrado"); $("ingresoCantidad").value=""; } });  
    ...
    safeClick("btnGuardarMantenimiento", () => { Data.guardarConfigMantenimiento($("mantenimientoAceite").value, $("mantenimientoBujia").value, $("mantenimientoLlantas").value); renderMantenimientoUI(); alert("Guardado"); });  
    safeClick("btnRegistrarServicio", () => { Data.registrarServicio($("ultimoAceiteKM").value, $("ultimoBujiaKM").value, $("ultimoLlantasKM").value); renderMantenimientoUI(); alert("Registrado"); });  
    ...
    safeClick("btnRegistrarDeudaFinal", () => { Data.agregarDeuda({ id:Date.now(), desc:$("deudaNombre").value, montoTotal:$("deudaMontoTotal").value, montoCuota:$("deudaMontoCuota").value, frecuencia:$("deudaFrecuencia").value, saldo:Number($("deudaMontoTotal").value) }); alert("Guardada"); window.location.reload(); });  
    safeClick("btnRegistrarAbono", () => { const id=$("abonoSeleccionar").value, m=$("abonoMonto").value; if(m && id) { const d=Data.getState().deudas.find(x=>x.id==id); if(d){ d.saldo-=safeNumber(m); Data.getState().movimientos.push({tipo:'gasto', fecha:new Date().toISOString(), desc:`Abono: ${d.desc}`, monto:safeNumber(m)}); Data.recalcularMetaDiaria(); alert("Abonado"); window.location.reload(); } } });  
