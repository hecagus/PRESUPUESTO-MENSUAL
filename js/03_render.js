...
function historyEvents(s){
  ...
  /* El combustible personal ya existe como movimiento de gasto. Solo agregamos aquí las cargas pagadas por un tercero para no duplicar dinero en Historial. */
  for(const c of (s.cargasCombustible||[]).filter(x=>x.pagador==='empresa')){const src=s.workSources.find(x=>x.id===c.sourceId);events.push({id:`fuel-${c.id}`,fecha:c.fecha,type:'fuel',title:`Combustible${c.gasolinera?` · ${c.gasolinera}`:''}`,meta:`${src?.name||'Empresa'} · ${safeFloat(c.litros).toFixed(2)} L · fondo de tercero`,amount:-safeFloat(c.costo),affectsPersonal:false,sourceId:c.sourceId});}
...
export function renderAdmin(){
  ...
  const business=$('businessZone');
  if(business){
    const enabled=caps.has(CAPABILITIES.BUSINESS);business.parentElement.classList.toggle('hidden',!enabled);
    if(enabled){
      const ingredients=s.business.ingredients||[],products=s.business.products||[];
      const ingredientList=ingredients.length?`<div style="margin-top:12px"><strong>Ingredientes / insumos</strong>${ingredients.map(i=>`<div style="display:flex;justify-content:space-between;gap:8px;align-items:center;padding:7px 0;border-bottom:1px solid #e2e8f0"><span>${i.name}<br><small>${fmtMoney(i.costPerUnit)} / ${i.unit}</small></span><button class="btn btn-outline" style="width:auto" data-action="update-ingredient" data-id="${i.id}">Actualizar costo</button></div>`).join('')}</div>`:'<small style="display:block;margin-top:10px;color:var(--text-sec)">Agrega ingredientes para empezar a costear recetas.</small>';
      const productList=products.length?`<div style="margin-top:14px"><strong>Productos</strong>${products.map(p=>`<div style="padding:8px 0;border-bottom:1px solid #e2e8f0"><strong>${p.name}</strong><br><small>Costo ${fmtMoney(costoProducto(p.id))} · venta ${fmtMoney(p.salePrice)} · margen ${fmtMoney(safeFloat(p.salePrice)-costoProducto(p.id))}</small><div class="grid-2" style="margin-top:6px"><button class="btn btn-outline" data-action="recipe-item" data-id="${p.id}">Receta</button><button class="btn btn-success" data-action="sale-product" data-id="${p.id}">Venta</button></div></div>`).join('')}</div>`:'<small style="display:block;margin-top:10px;color:var(--text-sec)">Crea productos y recetas para calcular costos y margen automáticamente.</small>';
      business.innerHTML=`<div class="grid-2"><button class="btn btn-outline" data-action="new-ingredient">+ Ingrediente</button><button class="btn btn-primary" data-action="new-product">+ Producto</button></div>${ingredientList}${productList}`;
    }
  }
}
