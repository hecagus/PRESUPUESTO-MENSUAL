...
export const renderDashboard = () => {
    ...
    if (tbodyTurnos) {  
        ...
        if (ultimos.length === 0) {  
            ...
            ultimos.forEach(t => {  
                tbodyTurnos.innerHTML += `<tr><td>${formatearFecha(t.fecha)}</td><td>${safeNumber(t.horas).toFixed(2)}h</td><td>${safeNumber(t.kmFinal)}</td><td>$${fmtMoney(t.ganancia)}</td></tr>`;  
            });  
        }  
    }  
    ...
    if (divGas) {  
        ...
        if (cargas.length === 0) {  
            ...
            cargas.forEach(c => {  
                html += `<tr><td>${formatearFecha(c.fecha)}</td><td>${c.litros} L</td><td>$${fmtMoney(c.costo)}</td><td>${c.km}</td></tr>`;  
            });  
            divGas.innerHTML = html + `</tbody></table>`;  
        }  
    }
};
...
export const renderHistorial = () => {
    ...
    movs.forEach(m => {  
        ...
        tr.innerHTML = `<td>${isIng?'➕':'➖'}</td><td>${formatearFecha(m.fecha)}</td><td>${m.desc}</td><td style="color:${isIng?'#16a34a':'#dc2626'}">$${fmtMoney(monto)}</td>`;  
        tbody.appendChild(tr);  
    });  

    resumen.innerHTML = `<p style="font-weight:bold; font-size:1.1rem;">Ingresos: <span style="color:#16a34a">$${fmtMoney(ing)}</span> | Gastos: <span style="color:#dc2626">$${fmtMoney(gas)}</span> | Neto: <span>$${fmtMoney(ing-gas)}</span></p>`;
};
...
export const setupAdminListeners = () => {
    ...
    safeClick("btnConfirmarFinalizar", () => {  
        ...
        renderTurnoUI(); renderOdometroUI();  
        $("gananciaBruta").value=""; $("kmFinalTurno").value="";   
        alert("Turno Finalizado");  
    });  
    safeClick("btnActualizarOdometro", () => {  
        const inputKm = $("inputOdometro");  
        if(inputKm && Data.actualizarOdometroManual(inputKm.value)) { renderOdometroUI(); inputKm.value = ""; alert("KM Actualizado"); }  
    });  
    const llenarCats = (tipo) => {  
        const sel = $("gastoCategoriaSelect"); const manualInput = $("gastoCategoriaManual"); if(!sel) return; sel.innerHTML=""; if(manualInput) manualInput.style.display = "none";  
        (CATEGORIAS_GASTOS[tipo]||[]).forEach(c => { const o = document.createElement("option"); o.value=c; o.text=c; sel.appendChild(o); });  
        sel.onchange = () => { if (sel.value.includes("➕") && manualInput) { manualInput.style.display = "block"; manualInput.focus(); } else if (manualInput) { manualInput.style.display = "none"; } };  
    };  
    ...
    safeClick("btnRegistrarGasto", () => {  
        ...
        let tipo = "moto"; const radios = document.getElementsByName("gastoTipoRadio"); for(let r of radios) if(r.checked) tipo = r.value;  
        const g = { id:Date.now(), fecha:new Date().toISOString(), categoria:cat, monto:Number(m), desc:$("gastoDescripcion").value, tipo:tipo };   
        if(esFijo) { g.frecuencia = $("gastoFrecuenciaSelect").value; Data.agregarGastoFijo(g); alert("Gasto Fijo Guardado"); }  
        else { g.frecuencia = "No Recurrente"; Data.agregarGasto(g); alert("Gasto Guardado"); }  
        $("gastoCantidad").value=""; $("gastoDescripcion").value=""; renderListasAdmin(); renderMetaDiaria(); if(esFijo) window.location.reload();  
    });  
    safeClick("btnRegistrarIngreso", () => {  
        const d = $("ingresoDescripcion").value; const m=$("ingresoCantidad").value; if(m) { Data.getState().movimientos.push({tipo:'ingreso', fecha:new Date().toISOString(), desc:d||"Ingreso Extra", monto:safeNumber(m)}); Data.saveData(); alert("Ingreso registrado"); $("ingresoCantidad").value=""; }  
    });  
    ...
    safeClick("btnRegistrarAbono", () => { const idDeuda = $("abonoSeleccionar").value; const m = $("abonoMonto").value; if(m && idDeuda) { const deuda = Data.getState().deudas.find(d => d.id == idDeuda); if(deuda) { deuda.saldo -= safeNumber(m); Data.getState().movimientos.push({tipo:'gasto', fecha:new Date().toISOString(), desc:`Abono: ${deuda.desc}`, monto:safeNumber(m)}); Data.recalcularMetaDiaria(); alert("Abono registrado"); window.location.reload(); } } });  
    safeClick("btnExportar", () => { if (typeof XLSX === 'undefined') return alert("Librería Excel no cargada."); const state = Data.getState(); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(state.turnos), "Turnos"); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(state.movimientos), "Movimientos"); XLSX.writeFile(wb, "Respaldo_Tracker.xlsx"); });  
