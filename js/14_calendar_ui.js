...
export function renderCalendarPage(){
  ...
  if($('calendarPosition'))$('calendarPosition').innerHTML=`
    ...
    <small style="display:block;margin-top:10px;color:var(--text-sec)">Comprometido = próximos pagos + presupuesto del hogar restante + otros compromisos + transporte público laboral estimado.</small>`;

  const events=upcomingFinancialEvents({days:45});
  if($('calendarEvents'))$('calendarEvents').innerHTML=events.length?events.map(e=>`<section class="card" style="margin:8px 0;border-left:4px solid ${e.type==='income'?'#16a34a':e.type==='goal'?'#6366f1':e.household?'#dc2626':'#f59e0b'}"><div style="display:flex;justify-content:space-between;gap:10px"><div><strong>${e.household?'🏠':typeIcon(e.type)} ${esc(e.title)}</strong><small style="display:block;color:var(--text-sec);margin-top:3px">${dateLabel(e.date)} · ${esc(e.category||'')}</small></div><strong>${e.amount===null?'—':fmtMoney(e.amount)}</strong></div></section>`).join(''):'<section class="card">No hay eventos financieros próximos.</section>';

  const commitments=state.financialPlan?.commitments||[];
  if($('calendarCommitments'))$('calendarCommitments').innerHTML=commitments.filter(c=>c.active!==false).length?commitments.filter(c=>c.active!==false).map(c=>`<section class="card"><div style="display:flex;justify-content:space-between;gap:8px"><div><strong>${esc(c.name)}</strong><small style="display:block;color:var(--text-sec)">${fmtMoney(c.amount)} · día ${esc(c.dueDay)} · ${esc(c.category)}</small></div><div style="text-align:right"><button class="btn btn-success" style="width:auto" data-calendar-action="pay" data-id="${c.id}">Pagar</button><button class="btn btn-outline" style="width:auto;margin-top:5px" data-calendar-action="pause" data-id="${c.id}">Pausar</button></div></div></section>`).join(''):'<section class="card">Tus gastos de casa se administran ahora desde 🏠 Hogar. Aquí sólo aparecen otros compromisos recurrentes.</section>';

  const budgets=householdBudgetStatus();
  if($('livingBudgetZone'))$('livingBudgetZone').innerHTML=budgets.length?budgets.map(b=>`<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #e2e8f0"><span>${esc(b.item.name)}</span><span><strong>${fmtMoney(b.remaining)}</strong> restantes de ${fmtMoney(b.budget)}</span></div>`).join(''):'<small>No configuraste presupuestos del hogar. Puedes hacerlo desde la pestaña Hogar.</small>';
...
export function renderCalendarPreview(){
  ...
  zone.innerHTML=`<section class="card" style="border-left:5px solid #0f766e"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><h2 style="margin:0">📆 Calendario financiero</h2><small style="color:var(--text-sec)">Próximos pagos, ingresos y metas.</small></div><a href="calendar.html" class="btn btn-outline" style="width:auto;text-decoration:none">Abrir</a></div><div style="margin:12px 0 8px"><strong>Realmente libre: ${fmtMoney(pos.free)}</strong></div>${events.length?events.map(e=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:5px 0"><span>${e.household?'🏠':typeIcon(e.type)} ${esc(e.title)}<br><small>${dateLabel(e.date)}</small></span><strong>${e.amount===null?'—':fmtMoney(e.amount)}</strong></div>`).join(''):'<small>Sin eventos próximos.</small>'}</section>`;
}

function newCommitment(refresh){
  Modal.show('Nuevo compromiso',[{label:'Nombre',key:'n',placeholder:'Escuela, pensión, cuota...'},{label:'Monto ($)',key:'m',type:'number'},{label:'Día del mes',key:'d',type:'number',value:1},{label:'Categoría',key:'c',type:'select',options:[{val:'Educación',txt:'Educación'},{val:'Salud',txt:'Salud'},{val:'Deuda',txt:'Deuda'},{val:'Vida',txt:'Otro'}]}],d=>run(()=>createCommitment({name:d.n,amount:d.m,dueDay:d.d,category:d.c}),refresh));
}
