/* v1.3.0 - Orquestación, eventos, PWA y sincronización. */
...
const ERROR_MESSAGES={KM_MENOR:'⛔ El kilometraje no puede ser menor al anterior.',KM_INVALIDO:'Ingresa un kilometraje válido mayor a 0.',SALDO_INVALIDO:'El saldo inicial no puede ser negativo.',MONTO_INVALIDO:'Ingresa un monto mayor a 0.',LITROS_INVALIDOS:'Ingresa una cantidad de litros mayor a 0.',GANANCIA_INVALIDA:'La ganancia no puede ser negativa.',DESCRIPCION_INVALIDA:'Escribe una descripción.',NOMBRE_INVALIDO:'Escribe un nombre válido.',TOTAL_INVALIDO:'El total de la deuda debe ser mayor a 0.',CUOTA_INVALIDA:'La cuota debe ser mayor a 0.',TURNO_NO_ACTIVO:'No hay una jornada activa para finalizar.',TURNO_YA_ACTIVO:'Ya hay una jornada activa.',TIPO_TRABAJO_INVALIDO:'Selecciona Jaimau o Uber.',ORIGEN_COMBUSTIBLE_REQUERIDO:'Selecciona si el repostaje corresponde a Jaimau o a uso personal/Uber.',BACKUP_INVALIDO:'El respaldo no es un JSON válido de esta aplicación.',COBRO_DUPLICADO:'Ya existe un cobro registrado para este periodo.'};
...
function registrarRepostaje(){
  const activo=Data.getState().turnoActivo?.tipoTrabajo||null;
  const fields=[];
  if(!activo)fields.push({label:'¿Quién paga este repostaje?',key:'pagador',type:'select',options:[{val:'empresa',txt:'💼 Jaimau · Ticket Car'},{val:'personal',txt:'🛵 Personal / Uber'}]});
  ...
  const title=activo==='jaimau'?'Repostaje Jaimau · Ticket Car':activo==='uber'?'Repostaje Uber · caja personal':'Repostaje de combustible';
  Modal.show(title,fields,d=>safe(()=>Data.registrarGasolina(d.l,d.c,d.k,d.pagador||null,d.e)));
}

function initAdminEvents(){
  $('btnTurnoIniciar')?.addEventListener('click',iniciarJornada);
  $('btnTurnoFinalizar')?.addEventListener('click',finalizarJornada);
  $('btnGasolina')?.addEventListener('click',registrarRepostaje);
  $('btnFondoJaimau')?.addEventListener('click',()=>Modal.show('Depósito Ticket Car Jaimau',[{label:'Importe depositado ($)',key:'m',type:'number'}],d=>safe(()=>Data.registrarFondoJaimau(d.m))));
