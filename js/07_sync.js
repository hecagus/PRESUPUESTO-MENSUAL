...
function mergeStates(local,remote){
  ...
  for(const key of ['turnos','movimientos','cargasCombustible','fondosCombustibleEmpresa','deudas','gastosFijosMensuales','ingresosFijos','workSources','accounts','assets','savingsGoals','automationRules','ruleApplications'])merged[key]=collectionMerge(local?.[key],remote?.[key]);
  ...
  merged.automationPreferences={...(remote?.automationPreferences||{}),...(local?.automationPreferences||{})};
...
async function signIn(){const button=document.getElementById('btnSyncLogin');try{if(button){button.disabled=true;button.textContent='Abriendo Google…';}setStatus('Abriendo acceso con Google…');const f=await loadFirebase(),provider=new f.GoogleAuthProvider(),result=await f.signInWithPopup(auth,provider);currentUser=result.user;authReady=true;renderSyncUI();if(navigator.onLine)await syncNow();}catch(e){showSyncError(e);if(button){button.disabled=false;button.textContent='Continuar con Google';}}}
...
export async function syncNow({forceLocal=false}={}){if(!FIREBASE_SYNC.enabled||!currentUser||syncing||!navigator.onLine)return;const currentHash=stateHash();if(observedHash!==null&&currentHash!==observedHash){observedHash=currentHash;meta={...meta,dirty:true};saveMeta(meta);}syncing=true;try{const f=await loadFirebase(),ref=docRef();const result=await f.runTransaction(db,async tx=>{const snap=await tx.get(ref),remote=snap.exists()?snap.data():null,remoteRevision=Number(remote?.revision)||0;if(remote&&remoteRevision>meta.baseRevision&&!forceLocal){if(meta.dirty)return{kind:'conflict',remote};return{kind:'pull',remote};}if(remote&&!forceLocal&&!meta.dirty&&remoteRevision===Number(meta.baseRevision||0))return{kind:'noop',revision:remoteRevision};const revision=Math.max(remoteRevision,Number(meta.baseRevision)||0)+1;tx.set(ref,{state:clone(Data.getState()),revision,updatedAt:new Date().toISOString(),deviceId:getDeviceId()});return{kind:'push',revision};});if(result.kind==='conflict'){conflictRemote=result.remote;renderSyncUI();return;}if(result.kind==='pull'){await applyRemote(result.remote);emitSyncComplete('pull');return;}observedHash=stateHash();meta={...meta,baseRevision:result.revision??meta.baseRevision,dirty:false,lastSync:new Date().toISOString()};saveMeta(meta);conflictRemote=null;renderSyncUI();emitSyncComplete(result.kind);}finally{syncing=false;}}
...
export async function initSync(){renderSyncUI();if(!FIREBASE_SYNC.enabled)return;observedHash=stateHash();if(meta.baseRevision===0&&!meta.lastSync&&hasMeaningfulLocalData()){meta={...meta,dirty:true};saveMeta(meta);}clearInterval(observerTimer);observerTimer=setInterval(observeState,FALLBACK_OBSERVER_MS);bindLifecycle();try{const f=await loadFirebase();f.onAuthStateChanged(auth,user=>{currentUser=user;authReady=true;renderSyncUI();if(user&&navigator.onLine)syncNow().catch(showSyncError);});}catch(e){authReady=true;renderSyncUI();showSyncError(e);}}