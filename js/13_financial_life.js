...
function personalCash(state){return(state.movimientos||[]).filter(m=>m.affectsPersonal!==false).reduce((a,m)=>m.tipo==='ingreso'?a+safeFloat(m.monto):m.tipo==='gasto'?a-safeFloat(m.monto):a,0);}
export function payCommitment(id,fecha=Date.now()){const state=getState(),c=ensureFinancialLife().commitments.find(x=>x.id===id&&x.active!==false);if(!c)throw new Error('COMPROMISO_NO_ENCONTRADO');const amount=positive(c.amount),d=new Date(fecha),period=monthId(d);if(c.lastPaidPeriod===period)throw new Error('COMPROMISO_YA_PAGADO');state.movimientos.push({id:uuid(),fecha:d.toISOString(),tipo:'gasto',desc:c.name,monto:amount,categoria:c.category||'Vida',accountId:PERSONAL_ACCOUNT_ID,affectsPersonal:true,commitmentId:c.id,periodo:period});c.lastPaidPeriod=period;saveData();return c;}

function pushMonthly(events,{id,title,amount,type,category,dueDay,sourceId},now,end){for(let offset=0;offset<3;offset++){const y=now.getFullYear(),m=now.getMonth()+offset,yy=y+Math.floor(m/12),mm=((m%12)+12)%12,day=dueDay==='fin_mes'?lastDay(yy,mm):clamp(Math.round(safeFloat(dueDay||1)),1,lastDay(yy,mm)),d=new Date(yy,mm,day,9,0,0);if(d>=now&&d<=end)events.push({id:`${id}-${d.toISOString().slice(0,10)}`,refId:id,date:d.toISOString(),dueDate:d.toISOString(),title,amount,type,category,sourceId:sourceId||null});}}
function pushBiweekly(events,base,now,end){for(let offset=0;offset<3;offset++){const raw=now.getMonth()+offset,y=now.getFullYear()+Math.floor(raw/12),m=((raw%12)+12)%12;for(const day of[15,lastDay(y,m)]){const d=new Date(y,m,day,9,0,0);if(d>=now&&d<=end)events.push({...base,id:`${base.id}-${d.toISOString().slice(0,10)}`,date:d.toISOString(),dueDate:d.toISOString()});}}}
function pushWeekly(events,base,weekDay,now,end){const target=Number(weekDay),normalized=Number.isFinite(target)?target:5,start=new Date(now);start.setHours(9,0,0,0);const diff=(normalized-start.getDay()+7)%7;start.setDate(start.getDate()+diff);for(let d=new Date(start);d<=end;d=new Date(d.getTime()+7*DAY))events.push({...base,id:`${base.id}-${d.toISOString().slice(0,10)}`,date:d.toISOString(),dueDate:d.toISOString()});}
function recurrence(events,base,frequency,dueDay,now,end){const f=String(frequency||'monthly').toLowerCase();if(['monthly','mensual'].includes(f))return pushMonthly(events,{...base,dueDay},now,end);if(['biweekly','quincenal'].includes(f))return pushBiweekly(events,base,now,end);if(['weekly','semanal'].includes(f))return pushWeekly(events,base,dueDay,now,end);if(['daily','diario'].includes(f)){for(let d=new Date(now);d<=end;d=new Date(d.getTime()+DAY))events.push({...base,id:`${base.id}-${d.toISOString().slice(0,10)}`,date:d.toISOString(),dueDate:d.toISOString()});}}
...
function pushSingleDebt(events,base,debt,now,end){const createdRaw=debt.creadaEn||debt.createdAt||now;let created=new Date(createdRaw);if(Number.isNaN(created.getTime()))created=new Date(now);let due=debt.dueDate?new Date(debt.dueDate):null;if(!due||Number.isNaN(due.getTime())){const target=clamp(Math.round(safeFloat(debt.diaPago||created.getDate())),1,31);let y=created.getFullYear(),m=created.getMonth();due=new Date(y,m,Math.min(target,lastDay(y,m)),9,0,0);if(due<created){m+=1;y+=Math.floor(m/12);m=((m%12)+12)%12;due=new Date(y,m,Math.min(target,lastDay(y,m)),9,0,0);}}if(due>end)return;const overdue=due<now,shown=overdue?new Date(now):new Date(due);shown.setHours(9,0,0,0);events.push({...base,id:`${base.id}-${due.toISOString().slice(0,10)}`,date:shown.toISOString(),dueDate:due.toISOString(),overdue});}
function debtPeriodId(debt,date){const d=new Date(date),f=String(debt.frecuencia||'Mensual').toLowerCase(),ym=monthId(d);if(/unico|único|single|one_time/.test(f))return`O:${debt.id}`;if(f.includes('diar'))return`D:${d.toISOString().slice(0,10)}`;if(f.includes('quinc'))return`${ym}-Q${d.getDate()<=15?1:2}`;if(f.includes('seman')){const s=new Date(d),day=s.getDay()||7;s.setDate(s.getDate()-day+1);return`W:${s.toISOString().slice(0,10)}`;}return ym;}
const debtPeriodPaid=(state,debt,date)=>{const period=debtPeriodId(debt,date);return(state.movimientos||[]).some(m=>m.debtId===debt.id&&m.debtPeriod===period);};
...
export function upcomingFinancialEvents({days=45,now=new Date()}={}){
  ...
  for(const d of state.deudas||[]){
    const saldo=safeFloat(d.saldo);if(!(saldo>0))continue;const cuota=Math.min(safeFloat(d.montoCuota),saldo),total=safeFloat(d.montoTotal),frequency=String(d.frecuencia||'').toLowerCase(),base={id:`debt-${d.id}`,refId:d.id,title:`Deuda · ${d.desc}`,amount:cuota,type:'debt',category:'Deuda'},single=['unico','único','single','one_time'].includes(frequency)||cuota>=saldo-0.005||(total>0&&safeFloat(d.montoCuota)>=total-0.005);
    if(single)pushSingleDebt(events,base,d,start,end);else{
      const tmp=[];recurrence(tmp,base,d.frecuencia,d.diaPago,start,end);events.push(...tmp.filter(e=>!debtPeriodPaid(state,d,e.dueDate||e.date)));
      if(['monthly','mensual'].includes(frequency))addMonthlyOverdue(events,base,d.diaPago,start,debtPeriodPaid(state,d,monthlyDueDate(d.diaPago,start)));
    }
  }
  ...
  for(const source of state.workSources||[]){if(source.active===false||source.status==='ended'||source.status==='paused')continue;const base={id:`income-${source.id}`,refId:source.id,title:`Ingreso esperado · ${source.name}`,amount:null,type:'income',category:'Trabajo',sourceId:source.id};if(source.compensation==='biweekly')pushBiweekly(events,base,start,end);else if(source.compensation==='monthly')pushMonthly(events,{...base,dueDay:source.paySchedule?.day||30},start,end);else if(source.compensation==='weekly')pushWeekly(events,base,source.paySchedule?.weekDay??5,start,end);}
  for(const goal of state.savingsGoals||[]){if(goal.active===false||!goal.targetDate||safeFloat(goal.reserved)>=safeFloat(goal.targetAmount))continue;const d=new Date(goal.targetDate);if(d>=start&&d<=end)events.push({id:`goal-${goal.id}`,refId:goal.id,date:d.toISOString(),title:`Meta · ${goal.name}`,amount:Math.max(0,safeFloat(goal.targetAmount)-safeFloat(goal.reserved)),type:'goal',category:'Meta'});}
...
export function livingBudgetStatus(now=new Date()){const plan=ensureFinancialLife(),state=getState(),start=new Date(now.getFullYear(),now.getMonth(),1),rows=[];for(const[key,budgetRaw]of Object.entries(plan.livingBudgets)){const budget=Math.max(0,safeFloat(budgetRaw));if(!(budget>0))continue;const spent=(state.movimientos||[]).filter(m=>m.tipo==='gasto'&&m.affectsPersonal!==false&&new Date(m.fecha)>=start&&categoryMatch(key,m)).reduce((a,m)=>a+safeFloat(m.monto),0);rows.push({key,budget,spent,remaining:Math.max(0,budget-spent)});}return rows;}
export function workTransportCommitment(now=new Date()){const state=getState(),daysInMonth=lastDay(now.getFullYear(),now.getMonth()),remainingDays=Math.max(0,daysInMonth-now.getDate()+1),fraction=remainingDays/daysInMonth;return(state.workSources||[]).filter(s=>s.active!==false&&s.status!=='ended'&&s.status!=='paused').reduce((sum,s)=>sum+publicTransportMonthlyCost(s)*fraction,0);}
export function financialPosition(now=new Date()){const state=getState();ensureFinancialLife();const cash=personalCash(state);let reserved=(state.savingsGoals||[]).filter(g=>g.active!==false).reduce((a,g)=>a+safeFloat(g.reserved),0);if(!(state.savingsGoals||[]).length)reserved=(state.wallet?.sobres||[]).filter(s=>s.categoria==='Ahorro'||s.categoria==='Meta').reduce((a,s)=>a+safeFloat(s.acumulado),0);const due=upcomingFinancialEvents({days:30,now}).filter(e=>['expense','debt'].includes(e.type)&&safeFloat(e.amount)>0).reduce((a,e)=>a+safeFloat(e.amount),0),living=livingBudgetStatus(now).reduce((a,x)=>a+x.remaining,0),workTransport=workTransportCommitment(now),committed=due+living+workTransport;return{cash,reserved,committed,free:cash-reserved-committed,due,living,workTransport};}
export function sourceCostProfile(sourceId,now=new Date()){ensureFinancialLife();const state=getState(),source=state.workSources.find(s=>s.id===sourceId);if(!source)return null;const monthStart=new Date(now.getFullYear(),now.getMonth(),1),income=(state.movimientos||[]).filter(m=>m.sourceId===sourceId&&m.tipo==='ingreso'&&m.categoria!=='Sistema'&&m.affectsPersonal!==false&&new Date(m.fecha)>=monthStart).reduce((a,m)=>a+safeFloat(m.monto),0),actualCosts=(state.movimientos||[]).filter(m=>m.sourceId===sourceId&&m.tipo==='gasto'&&m.affectsPersonal!==false&&new Date(m.fecha)>=monthStart).reduce((a,m)=>a+safeFloat(m.monto),0),publicTransport=publicTransportMonthlyCost(source);return{source,income,actualCosts,publicTransport,estimatedNet:income-actualCosts-publicTransport};}
