/* v1.3.0 - Estado, persistencia y dominio. Cero DOM. */
...
export function registrarFondoJaimau(monto){const m=requirePositive(monto);store.fondosCombustibleEmpresa.push({id:uuid(),fecha:new Date().toISOString(),monto:m,fuente:'jaimau',tipo:'deposito',desc:'Depósito de su Empresa'});return commit();}
...
function resolverPagadorCombustible(pagador){const activo=store.turnoActivo?.tipoTrabajo;if(activo==='jaimau')return'empresa';if(activo==='uber')return'personal';if(pagador==='empresa'||pagador==='personal')return pagador;throw new Error('ORIGEN_COMBUSTIBLE_REQUERIDO');}
export function registrarGasolina(litros,costo,km,pagador=null,gasolinera=''){
  ...
  const resuelto=resolverPagadorCombustible(pagador),empresa=resuelto==='empresa',activo=store.turnoActivo?.tipoTrabajo||null,contexto=activo||(empresa?'jaimau':'personal'),fecha=new Date().toISOString(),estacion=String(gasolinera||'').trim();
  store.cargasCombustible.push({id:uuid(),fecha,litros:l,costo:c,km:k,pagador:resuelto,fuente:empresa?'jaimau':(contexto==='uber'?'uber':'personal'),tipoTrabajo:contexto,gasolinera:estacion});
  if(!empresa){const desc=estacion?`⛽ Gasolina · ${estacion}`:'⛽ Gasolina';store.movimientos.push({id:uuid(),fecha,tipo:'gasto',desc,monto:c,categoria:'Operativo',fuente:contexto==='uber'?'uber':'personal'});const gas=store.wallet.sobres.find(s=>s.categoria==='Operativo'&&/gas|combustible/i.test(String(s.desc||'')));if(gas)gas.acumulado=Math.max(0,safeFloat(gas.acumulado)-c);}
