...
export const loadData = () => {
    ...
    recalcularMetaDiaria(); // Recalcular al cargar
};
...
// --- LÓGICA CRÍTICA: META DIARIA (TU SOLICITUD) ---
...
export const finalizarTurnoLogic = (ganancia) => {
    ...
    if (nuevoTurno.ganancia > 0) {
        state.movimientos.push({
            tipo: 'ingreso', fecha: fin.toISOString(), desc: 'Turno Finalizado', monto: nuevoTurno.ganancia
        });
    }
...
export const registrarCargaGasolina = (litros, costo, kmActual) => {
    ...
    state.parametros.ultimoKM = safeNumber(kmActual);
    saveData();
};

// --- CRUD GENERICS ---
export const agregarGasto = (gasto) => { state.gastos.push(gasto); state.movimientos.push(gasto); saveData(); };
export const agregarGastoFijo = (gasto) => { state.gastosFijosMensuales.push(gasto); recalcularMetaDiaria(); };
export const agregarDeuda = (deuda) => { state.deudas.push(deuda); recalcularMetaDiaria(); };
