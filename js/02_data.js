...
let turnoActivo = JSON.parse(localStorage.getItem("turnoActivo_v3")) || null;
...
/* --- EXPORTACIONES DE LÓGICA (AQUÍ ESTÁ TODO) --- */

// 1. Turnos
export const iniciarTurnoLogic = () => {
    if (turnoActivo) return false;
    turnoActivo = { inicio: new Date().toISOString() };
    localStorage.setItem("turnoActivo_v3", JSON.stringify(turnoActivo));
    return true;
};
...
export const finalizarTurnoLogic = (ganancia) => {
    ...
    state.turnos.push(t);
    if(t.ganancia > 0) state.movimientos.push({ tipo: 'ingreso', fecha: fin.toISOString(), desc: 'Turno Finalizado', monto: t.ganancia });
    
    turnoActivo = null; 
    localStorage.removeItem("turnoActivo_v3"); 
    saveData();
};
