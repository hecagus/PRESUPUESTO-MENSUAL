...
import {
  ensureHousehold,householdItems,householdSummary,householdBudgetStatus,householdMonthlyEquivalent,
...
function spentFor(id){const row=householdBudgetStatus().find(x=>x.item.id===id);return row||null;}
function itemStatus(item){
  ...
  if(item.priority==='obligatory')return `${freqLabel(item.frequency)} · ${item.frequency==='variable'?'sin fecha':`día ${item.dueDay}`}`;
...
export function renderHome(){
  ...
  $('homeSummary').innerHTML=`<div class="grid-2"><div><small>🔴 Obligatorio / mes</small><strong style="display:block;font-size:1.2rem">${fmtMoney(summary.mandatory)}</strong></div><div><small>🟡 Presupuestado / mes</small><strong style="display:block;font-size:1.2rem">${fmtMoney(summary.budgeted)}</strong></div><div><small>💸 Gastado este mes</small><strong style="display:block;font-size:1.2rem">${fmtMoney(summary.spent)}</strong></div><div><small>🔵 Discrecional gastado</small><strong style="display:block;font-size:1.2rem">${fmtMoney(summary.discretionarySpent)}</strong></div></div>`;
...
function itemModal(item,refresh){
  ...
  setTimeout(()=>{
    ...
    const set=(key,value)=>{const el=body.querySelector(`[data-k="${key}"]`);if(el)el.value=value;};
    set('category',item?.category||'Servicios');set('priority',item?.priority||'obligatory');set('frequency',item?.frequency||'monthly');
  },0);
}

function run(fn,refresh){try{fn();refresh?.();document.dispatchEvent(new CustomEvent('budget:data-changed'));}catch(e){console.error(e);const map={MONTO_INVALIDO:'Ingresa un monto mayor a 0.',NOMBRE_INVALIDO:'Escribe un nombre.',GASTO_HOGAR_NO_ENCONTRADO:'No se encontró ese gasto.',GASTO_HOGAR_YA_PAGADO:'Ese pago ya quedó registrado para este periodo.'};alert(map[e.message]||'No se pudo completar la operación.');}}

function record(item,refresh){
  const label=item.priority==='obligatory'?'Importe pagado ($)':'Importe gastado ($)';
  Modal.show(`${actionLabel(item)} · ${item.name}`,[{label,key:'amount',type:'number',value:item.amount}],d=>run(()=>recordHouseholdExpense(item.id,d.amount),refresh));
}
