/* v2.7.2 - UI de Hogar: completados fuera de pausados y calendario único de pago. */
...
function isCompletedOneTime(item){
  return item.frequency==='one_time'&&(getState().movimientos||[]).some(m=>m.householdExpenseId===item.id);
}
...
function schedulePayload(data,item){
  const d={...data},date=String(d.nextDueDate||'').trim();
  if(d.frequency==='variable')d.nextDueDate='';
...
function itemModal(item,refresh){
  Modal.show(item?'Editar gasto del hogar':'Nuevo gasto del hogar',[
    ...
    {label:'Próximo pago / fecha del gasto',key:'nextDueDate',type:'date',value:scheduleDateValue(item)}
  ],d=>run(()=>{const payload=schedulePayload(d,item);return item?updateHouseholdExpense(item.id,payload):createHouseholdExpense(payload);},refresh));
  setTimeout(()=>{
    ...
    const frequency=body.querySelector('[data-k="frequency"]'),date=body.querySelector('[data-k="nextDueDate"]'),wrap=date?.parentElement,label=wrap?.querySelector('label');
    const syncScheduleField=()=>{if(!frequency||!date||!wrap||!label)return;const f=frequency.value;wrap.style.display=f==='variable'?'none':'block';label.textContent=f==='one_time'?'Fecha del gasto (opcional)':'Próximo pago / inicio de frecuencia (opcional)';if(f==='variable')date.value='';};
    frequency?.addEventListener('change',syncScheduleField);syncScheduleField();
  },0);
}
