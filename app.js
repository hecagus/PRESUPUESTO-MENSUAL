...
//   GUARDAR PARAMETROS PANEL
// =========================
function guardarPanelData() {
    const meta = Number($("metaTotal").value) || 0;
    const dias = Number($("diasRestantes").value) || 0;
    const gxh = Number($("gananciaPorHora").value) || 0;
    const gf = Number($("gastosFijos").value) || 0;
...
// =========================
//   GUARDAR / EDITAR DEUDAS
// =========================
function guardarNuevaDeuda() {
    const nombre = $("deudaNombre").value.trim();
    const monto = Number($("deudaMonto").value);
    const pago = Number($("deudaPagoMensual").value);

    if (!nombre || !monto || !pago) return;
...
function registrarAbono() {
    const deudaId = $("abonoDeudaId").value;
    const cantidad = Number($("abonoCantidad").value);

    if (!deudaId || !cantidad) return;

    const deuda = panelData.deudas.find(d => d.id == deudaId);
    if (!deuda) return;

    deuda.restante = Math.max(0, Number(deuda.restante) - cantidad);

    panelData.historial.push({
        id: Date.now(),
        tipo: "Gasto",
        descripcion: `Abono a ${deuda.nombre}`,
        fecha: new Date().toISOString(),
