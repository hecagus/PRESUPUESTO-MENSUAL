...
test('rendimiento de gasolina usa kilómetros entre cargas y litros de la carga actual',()=>{
  ...
  assert.equal(r.segments.length,2);assert.equal(r.last.km,120);assert.equal(r.last.kmL,24);
  assert.equal(Math.round(r.avgKmL*10)/10,24.4);assert.equal(Math.round(r.last.costoKm*100)/100,1.04);
});
...
test('costos operativos recientes no mezclan Hogar, deuda ni gasolina',()=>{
  ...
  assert.deepEqual(gastosOperativosRecientes(store).map(x=>x.id),['op']);
});
...
test('PWA v3 captura prompt temprano, registra SW temprano y no finge instalabilidad',async()=>{
  ...
  assert.doesNotMatch(pwa,/button\.textContent=deferredPrompt\?'Instalar ahora':'Comprobar instalación'/);
  assert.equal(manifest.id,'/index.html');assert.equal(manifest.start_url,'/index.html');assert.equal(manifest.scope,'/');assert.equal(manifest.prefer_related_applications,false);
  assert.ok(manifest.icons.some(x=>x.sizes==='192x192'&&x.purpose==='any'));assert.ok(manifest.icons.some(x=>x.sizes==='512x512'&&x.purpose==='any'));
  assert.doesNotMatch(admin,/id="appInstallCard"/);
  assert.ok(vercel.headers.some(x=>x.source==='/sw.js'));
});
